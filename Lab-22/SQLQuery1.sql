...
(
    SELECT CA.*,
           C.AVG_AMOUNT,
           ABS(CA.AMOUNT - C.AVG_AMOUNT) AS DIFF_AMOUNT
    FROM CUSTOMER_ALL CA
    INNER JOIN CTE_CLOSET C
        ON CA.CATEGORY = C.CATEGORY
)
SELECT TOP 1 *
FROM CTE_DIFFERENCE
...
(
  SELECT * ,
  DENSE_RANK() OVER(ORDER BY AMOUNT) AS D_RANK,
  RANK() OVER(ORDER BY AMOUNT) AS N_RANK
  FROM CUSTOMER_ALL
)
SELECT *
FROM CTE_DIFF
...
--(Ques 20 can be understand in two different types)

--(i)Display customers whose amount difference from category topper is minimum.
--Then the minimum possible difference is 0, and the topper itself has difference 0.
WITH CTE_TOPPER AS (
    SELECT * , 
	       MAX(AMOUNT) 
...
--(ii)Display the closest customer to the category topper, excluding the topper itself
WITH CTE_TOPPER AS
(
...
CTE_DIFF AS
(
    SELECT *,MAX_AMOUNT - AMOUNT AS AMOUNT_DIFF
    FROM CTE_TOPPER
