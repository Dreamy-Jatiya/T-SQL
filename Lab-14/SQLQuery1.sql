...
SELECT D.DEPARTMENTNAME , MIN(SALARY) AS MIN_SALARY , MAX(SALARY) AS MAX_SALARY 
FROM DEPARTMENT D INNER JOIN PERSON P
ON D.DEPARTMENTID = P.DEPARTMENTID
GROUP BY D.DEPARTMENTNAME

--13. Find city wise total, average, maximum and minimum salary. 
SELECT P.CITY , SUM(SALARY) AS TOTAL_SALARY , AVG(SALARY) AS AVG_SALARY , MAX(SALARY) AS MAX_SALARY  , MIN(SALARY) AS 
MIN_SALARY
FROM DEPARTMENT D INNER JOIN PERSON P
ON D.DEPARTMENTID = P.DEPARTMENTID
GROUP BY P.CITY
...
SELECT AVG(SALARY) AS AVG_SALARY_AHMEDABAD
FROM DEPARTMENT D INNER JOIN PERSON P
ON D.DEPARTMENTID = P.DEPARTMENTID
WHERE P.CITY = 'AHMEDABAD'
...
--single column) 
SELECT CONCAT(P.PERSONNAME,' LIVES IN ',P.CITY,' AND WORKS IN ',ISNULL(D.DEPARTMENTNAME,'NONE') )
...
--single column) 
SELECT CONCAT(P.PERSONNAME,' EARNS ',P.SALARY,' FROM ',D.DEPARTMENTNAME)
FROM DEPARTMENT D INNER JOIN PERSON P
ON D.DEPARTMENTID = P.DEPARTMENTID

--17. Find city & department wise total, average & maximum salaries. 
SELECT AVG(P.SALARY) AS AVG_SALARY , MAX(P.SALARY) AS MAX_SALARY
FROM DEPARTMENT D INNER JOIN PERSON P
ON D.DEPARTMENTID = P.DEPARTMENTID
...
SELECT  P.PERSONNAME
FROM DEPARTMENT D RIGHT JOIN PERSON P
ON D.DEPARTMENTID = P.DEPARTMENTID
...
FROM DEPARTMENT D FULL OUTER JOIN PERSON P
ON D.DEPARTMENTID = P.DEPARTMENTID
GROUP BY D.DEPARTMENTNAME
